...
<h1 style="color:white;font-size:50px;text-align:center" >##JOiN##</h1>
...
<div id="frm" align="center">
			...
			<table>
			 ...
			 <td><input type="tel" id="contact1"  name="contact1" pattern="^\d{10}$" required></td>
			 </tr>
			...
			<td> <textarea rows="4" cols="30" id="experience" name="experience">
			 </textarea></td>
			 </tr>
            <tr>
			 <td>
...
if(!empty($_POST['username'])&& !empty($_POST['email']) && !empty($_POST['address']) && !empty($_POST['experience']) && !empty($_POST['qualification'])){  
    $user=$_POST['username'];    
	$email=$_POST['email'];
	$add=$_POST['address'];
...
else
{
  echo "not succesful";
   ...
   if($numrows!=0)  
    {  
    $sql="INSERT INTO book(username,email,address,experience,qualification) VALUES('$user','$email','$add','$experience','$qualification')";  
